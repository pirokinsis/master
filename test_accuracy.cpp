...
int main(int argc, char **argv)
{
    ...
    for (double j = 0.0001; j < 1; j *= pow(10000, 0.01))
    {
        ...
        if (active_vars_prev.sum() != active_vars_lasso.sum())
        {
            ...
            cout << i << endl;
            cout << "Result:" << endl;
            cout << active_vars_lasso - Eigen::MatrixXd::Identity(n,n) << endl;
    ...
    for (double j = 0.0001; j < 1; j *= pow(10000, 0.01))
    {
        ...
        if (active_vars_prev.sum() != active_vars_group.sum())
        {
            cout << "Lambda: " << j << endl;
            cout << "Desired:" << endl;
            cout << ((i.array() + i.transpose().array()) > 0).cast<double>() << endl;
            cout << "Result:" << endl;
            cout << active_vars_group - Eigen::MatrixXd::Identity(n,n) << endl;
